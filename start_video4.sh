...
if [ -z "$1" ]; then
    ...
    DEVICE=$(cat ~/TecXotic_CameraStream/vidformat.param | xargs | cut -f6 -d" ")
else
    WIDTH=$1
    HEIGHT=$2
    FRAMERATE=$3
...
gstOptions1="$(sed '1q;d' ~/gstreamer2.param) $(sed '2q;d' ~/gstreamer2.param) $(sed '3q;d' ~/gstreamer2.param) $(sed '7q;d' ~/gstreamer2.param)"
